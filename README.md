...
- 流表容量仍固定为256个槽位；已有流在满载时仍可更新，新流数据包会计入`flow_rejected`而不终止程序；实时过期可以复用旧槽位，但尚未实现动态扩容、负载因子控制和满载驱逐；
...
- ARM Linux开发板已完成原生Debug/Release构建、当前17项CTest、离线跨平台一致性、两种交叉产物、物理网卡抓包、单流/多流性能、满载边界和10分钟长稳基线；128流约9 Kpps持续540万包时零drop且RSS稳定。
