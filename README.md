...
实时模式还会按单调时钟约每5秒输出一次运行指标，包括区间包数、完整/截断/畸形/不支持/流表拒绝分类、捕获/线路字节数、PPS、Mbps、活动流数量、流表占用率和累计过期流数量。采集句柄使用非阻塞模式，主循环通过`poll()`等待网卡可读或最多1秒超时，因此即使没有任何匹配包，也能定期输出零流量报告，同时避免忙轮询持续占用CPU。

程序退出时还会输出流表线性探测统计：数据包路径操作次数、实际检查槽位总数、平均探测长度、最大探测长度和计数是否饱和。`flow_table_find()`等管理查询不进入该统计，满载新流最终返回`ENOSPC`之前发生的完整扫描仍会记录，因此可以区分正常查找成本与容量拒绝成本。
...
- 流表容量仍固定为256个槽位；已有流在满载时仍可更新，新流数据包会计入`flow_rejected`而不终止程序；实时过期可以复用旧槽位，但尚未实现动态扩容和满载驱逐。当前已能观测累计平均与最大探测长度；LubanCat上128流、50%占用率实测为`average=1.00`、`maximum=1`，说明当前受控工作负载没有查找退化，但不消除256条同时活跃流的容量边界；
...
- ARM Linux开发板已完成原生Debug/Release构建、当前17项CTest、离线跨平台一致性、两种交叉产物、物理网卡抓包、单流/多流性能、满载边界、流表探测成本和10分钟长稳基线；128流约9 Kpps持续540万包时零drop且RSS稳定。
...
1. 使用已取得的多流、占用率和`flow_rejected`数据决定是否增加流表探测长度指标、满载驱逐、重建或动态扩容；
2. 完成非root抓包权限和服务化运行，或进入TCP状态跟踪与流重组；
