...
实时模式还会按单调时钟约每5秒输出一次运行指标，包括区间包数、捕获/线路字节数、PPS、Mbps、活动流数量、流表占用率和累计过期流数量。采集句柄使用非阻塞模式，主循环通过`poll()`等待网卡可读或最多1秒超时，因此即使没有任何匹配包，也能定期输出零流量报告，同时避免忙轮询持续占用CPU。
...
| `runtime_metrics` | 根据单调时钟和累计计数生成区间PPS、Mbps、流表占用率及过期流指标 |
