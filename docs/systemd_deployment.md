...
当前阶段不重复进行一小时或数小时浸泡。此前服务已经连续运行数天，能够证明进程保持运行，但由于最终发生根分区容量事故，这段经历不作为严格受控的生产级性能报告。若未来需要对外声明生产级长期稳定性，再使用明确自动结束条件补做，并观察journal占用、内存、CPU和drop。

当前仍需在下一次受控重启后复核journal drop-in继续生效，且分析器保持预期的禁用状态；该检查可以随正常维护完成，不为它单独安排一次重启。
