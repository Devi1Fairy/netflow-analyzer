...
- 当前分支在既有非root systemd、TCP状态和流表生命周期功能之上，已提交流特征模型、版本化特征CSV、CLI参数、文件生命周期、数据集标签审计、身份规范化、稳定样本ID和元数据sidecar；本次文档更新前最新已推送提交为`52cc225 docs(ml): record IoT-23 flow-boundary pilot`，实际接手时仍须以`git log`为准；
...
- 第二个试点IoT-23 v2 Scenario 3-1已有完整标签文件审计、身份规范化和候选索引：真实156103条标签全部转换成功，`identity_unsupported=0`，其中73944条单包记录为零时长；标签组为151567条恶意、4536条正常、0条排除。索引包含81404个规范化双向流键，其中71026个键有多条候选、单键最多185条。定向8包PCAP试点在默认聚合下得到2条同标签歧义流；3秒空闲分段后得到与前四条Zeek记录逐条对应的4条唯一恶意候选。索引模块已有四状态纯内存流区间分类函数，并通过现有CTest的合成用例；8包判断仍来自临时只读诊断，正式CSV匹配审计器和全场景质量尚未完成，不能把标签数或索引键数误称为可训练样本数。
...
应用处理结果分类、满载策略、线性探测可观测性和TCP状态跟踪已经完成。第一版流特征链把完整流记录转换为版本化模型输入；过期、淘汰和最终剩余记录在各自生命周期结束时写出。CTU-13 Scenario 7匹配审计发现截断PCAP和Argus DNS事务边界导致不能可靠一对一连接。第二个试点IoT-23 v2 Scenario 3-1已完成标签审计、精确微秒、双向身份规范化和候选索引：156103条标签对应81404个流键，71026个键有多条候选；8包定向试点通过3秒分段得到4条与前四条Zeek记录一致的流，临时审计均为唯一恶意候选。四状态纯内存分类函数已由合成测试覆盖，x86_64 Debug现有27项CTest全部通过。IoT-23的正式CSV匹配工具和全场景质量仍待实现，不能把标签或流键数量当作可训练样本数。ARM64板端仍以之前18项和真实TCP、systemd、性能、存储恢复验收为已验证基线；新增特征尚未上板。训练、评估和在线推理均未实现。`labs/thread_pipeline`仍因现有性能数据未显示单线程瓶颈而不接入正式路径。
