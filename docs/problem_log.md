...
### 6.1 CTU-13截断PCAP和离线流边界导致标签无法一对一连接

现象：

完成Scenario 7标签侧时间、协议和双向五元组规范化后，使用相同流键和闭时间区间重叠连接本项目普通流CSV。真实试点没有得到可直接用于训练的唯一匹配：

- 前1000包形成47条C流，保守监督策略下47条全部为`unmatched`；
- 抽取感染主机`147.32.84.165`的UDP/53流量后，45个完整包形成2条C流，两条都是`ambiguous_same_label`，即每条C流同时覆盖多个恶意Argus标签；
- 首条正常TCP标签的首包能在PCAP中找到，但分析器把它归为`truncated`，没有建立对应TCP流。

原因：

这是两个独立的数据语义边界叠加，并非简单的时间格式或端点方向错误。

1. 公开PCAP按协议截断。上述TCP包的`caplen=54`，Ethernet头14字节、IPv4头20字节，TCP数据只剩20字节；该包的TCP头因选项实际需要32字节。捕获内存中没有剩余12字节，解析器按照`caplen`拒绝继续并返回`truncated`是正确的安全行为。
2. 当前离线分析语义是在整份输入结束前持续聚合同一双向五元组，不执行实时模式的空闲过期扫描。CTU-13的Argus标签会把同一五元组按自己的超时和生命周期规则拆成多条记录。因此一条C流可能在时间上覆盖多条同标签记录，仅凭五元组与区间重叠无法恢复唯一对应关系。
3. 前1000包的47条流在包含全部标签类别的补充诊断中均唯一对应背景记录；正式审计按既定监督策略先排除112345条背景或不确定标签，所以这些流显示为未匹配是预期行为，不应把背景强行标成正常。

处理与结果：

- 新增独立匹配审计器，先按规范化双向五元组建立候选索引，再检查闭时间区间重叠；
- 输出`unique`、`unmatched`、`ambiguous_same_label`和`ambiguous_conflicting_labels`四类结果，多个同标签候选也不自动选择；
- `exclude`标签在身份解析前跳过，不让背景数据中的缺失端口中断监督样本审计；ARP等当前不支持协议单独计数；
- 合成测试覆盖唯一恶意、唯一正常、排除、不支持、未匹配和两类歧义，全量x86_64 Debug CTest为23/23通过；
- 真实前1000包审计得到`flows_total=47`、`matches_unmatched=47`，其余匹配类别为0；真实DNS子集得到`flows_total=2`、`matches_ambiguous_same_label=2`，没有冲突标签歧义。

结论：

匹配工具的职责不是尽可能多地产生标签，而是测量哪些样本能够被可靠监督。当前结果阻止了把背景流误标或从多个Argus生命周期中任意挑选标签。下一步应先设计离线流分段、生命周期和`sample_id`元数据契约；不能通过忽略`caplen`、扩大时间容差或默认选择第一个候选来制造训练样本。

### 6.2 空闲阈值扫描无法重建Argus的DNS事务边界

现象：

完成离线事件时间空闲分段和样本元数据sidecar后，从Scenario 7公开截断PCAP中提取受感染主机`147.32.84.165`参与的UDP/53流量。子集包含45包，持续约300.905秒；官方监督标签中有21条对应的恶意DNS记录。分别关闭分段以及使用`1/3/5/10/30/60/120`秒空闲阈值运行分析器和匹配审计，结果如下：

| 空闲阈值 | C流总数 | 唯一匹配 | 未匹配 | 同标签歧义 | 冲突标签歧义 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 关闭 | 2 | 0 | 0 | 2 | 0 |
| 1秒 | 10 | 5 | 0 | 5 | 0 |
| 3秒 | 7 | 4 | 0 | 3 | 0 |
...
1. 空闲分段确实改善了部分匹配，证明原来的整文件五元组聚合会把相隔很久的生命周期合并；但最激进的1秒阈值也只产生10条C流和5条唯一匹配，仍不能恢复21条官方DNS记录。
2. 5秒与3秒都得到4条唯一匹配，而5秒少产生1条C流，因此5秒可以作为当前项目流定义的后续实验候选；它不是官方标签生成参数，也不能据此宣称恢复了Argus语义。
3. Scenario 7官方`argus.conf`显式设置的是`ARGUS_FLOW_STATUS_INTERVAL=3600`，它表示长流的周期状态报告间隔，不是空闲超时。协议空闲计时器没有在该文件中显式覆盖，只保留默认参考注释；普通IP/UDP的30秒参考值仍不足以解释DNS标签边界。
4. 标签中同一UDP五元组在`13:56:34.950513`和`13:56:34.951771`分别开始两条记录，间隔只有约1.258毫秒。如此短的间隔不可能由1秒或30秒空闲超时造成，说明Argus还使用DNS事务或协议状态划分记录。
5. 公开UDP捕获的`caplen`为42字节，只含Ethernet 14字节、IPv4 20字节和UDP 8字节；DNS头及事务ID已经从文件中删除。缺失的是输入数据，不只是尚未实现的解析代码，因此补写DNS解析器也无法从该截断文件恢复事务身份。

处理与结论：

- 保留`--flow-idle-timeout`作为本项目自己的可复现生命周期配置，它对真实离线分析仍有价值；
- 不再通过穷举更多秒数尝试复制该DNS子集的21条Argus记录；
- 继续把非唯一匹配标为不可训练，不能因为多个候选都是`malicious`就任意选择；
- 后续训练数据方案必须显式选择：按本项目流定义保守聚合监督信息、改用保留所需报文头的数据集，或者使用与公开标签同源的现成流特征；
- 本次实验只说明空闲阈值对这45包恶意DNS子集的影响，不能把5秒推广成所有UDP、所有CTU-13场景或实时部署的最佳阈值。
